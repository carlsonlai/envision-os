...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  if (error || !code) {
    return html(`<!DOCTYPE html><html><body style="background:#0a0a0f;color:#f87171;font-family:monospace;padding:2rem;"><h2>Google Error</h2><p>${error ?? 'No code'}</p></body></html>`)
  }
...
<html lang="en"><head><meta charset="utf-8"><title>YouTube Connected</title></head>
<body style="background:#0a0a0f;color:#e4e4e7;font-family:monospace;padding:2rem;max-width:640px;margin:0 auto;">
  <h2 style="color:#34d399;">&#10003; YouTube / Google Authorized</h2>
  ...
  <div style="background:#18181b;border:1px solid #3f3f46;border-radius:8px;padding:1.25rem;margin:1.5rem 0;">
    <p style="color:#34d399;margin:0.25rem 0;word-break:break-all;">GOOGLE_ACCESS_TOKEN=${tokenData.access_token}</p>
    ${tokenData.refresh_token ? `<p style="color:#818cf8;margin:0.25rem 0;word-break:break-all;">GOOGLE_REFRESH_TOKEN=${tokenData.refresh_token}</p>` : ''}
    <p style="color:#71717a;margin:0.5rem 0 0.25rem;"># Find your channel ID: youtube.com → Your channel → URL bar</p>
    <p style="color:#818cf8;margin:0.25rem 0;">YOUTUBE_CHANNEL_ID=&lt;UC...&gt;</p>
  </div>
